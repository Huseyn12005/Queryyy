...
{
    context.Debtors.Add(debtors[i]);
}
context.SaveChanges();

//  2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag

var a = context.Debtors.ToList().FindAll(x => x.Email.EndsWith("rhyta.com") || x.Email.EndsWith("dayrep.com"));
...
var b = context.Debtors.ToList().FindAll(x => date - x.BirthDay.Year >= 26 && date - x.BirthDay.Year <= 36);
...
var c = context.Debtors.ToList().FindAll(x => x.Debt <= 5000);
...
var d = context.Debtors.ToList().Where(x => x.FullName.Length > 18 && x.Phone.Count(c => c == '7') >= 2);

foreach (var debtor in d)
{
...
var f = context.Debtors.ToList().Where(debtor => debtor.BirthDay.Month == 12 || debtor.BirthDay.Month == 1 || debtor.BirthDay.Month == 2);

foreach (var debtor in f)
{
    Console.WriteLine(debtor);
}
...
double z = context.Debtors.ToList().Average(debtor => debtor.Debt);
Console.WriteLine(z);
...
var q = context.Debtors.ToList().Where(x => x.Phone.Count(c => c == '8') == 0);

foreach (var debtor in q)
{
    Console.WriteLine($"{debtor.FullName}-{date1 - debtor.BirthDay.Year}-{debtor.Debt}");
}
...
var g = context.Debtors.ToList().Where(debtor =>
 debtor.FullName.ToLower().GroupBy(x => x).Any(c => c.Count() >= 3)
).OrderBy(debtor => debtor.FullName);

foreach (var debtor in g)
{
    Console.WriteLine(debtor);
}
...
var Il = context.Debtors.ToList().GroupBy(debtor => debtor.BirthDay.Year)
...
var Debtors5 = context.Debtors.ToList().OrderByDescending(debtor => debtor.Debt).Take(5);

foreach (var debtor in Debtors5)
{
    Console.WriteLine(debtor);
}
...
var Total = context.Debtors.ToList().Sum(debtor => debtor.Debt);
Console.WriteLine(Total);

//	16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

var Veteran = context.Debtors.ToList().FindAll(debtor => debtor.BirthDay.Year < 1939 || debtor.BirthDay.Year > 1945);

foreach (var debtor in Veteran)
{
    Console.WriteLine(debtor);
}
...
var number = context.Debtors.ToList().Where(debtor => debtor.Phone.GroupBy(c => c).Any(g => g.Count() == 1));

foreach (var debtor in number)
{
...
var smile = context.Debtors.ToList().Where(x => x.FullName.Count(c => c == 's') > 0 && x.FullName.Count(c => c == 'm') > 0
                                            && x.FullName.Count(c => c == 'i') > 0 && x.FullName.Count(c => c == 'l') > 0 && x.FullName.Count(c => c == 'e') > 0);

foreach (var debtor in smile)
{
    Console.WriteLine(debtor);
}